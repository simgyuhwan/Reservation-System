...
**MSA 아키텍처**는 여러 개의 작고 독립적인 서비스로 분리하고 각 서비스간 통신하도록 구성되어 있다. 그렇기 때문에 서비스 간의 복잡도와 의존성이 높아지고 장애가 발생할 확률도 높아진다.

그러므로 모니터링을 구성하는 것은 **필수**라고 볼 수 있다. 간단한 설정 방법이나 local(msa) + local(grafana + prometheus)를 구성하는 방법은 참고 링크로 모아놨다.
